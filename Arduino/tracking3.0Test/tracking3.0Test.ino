...
int response() {
  ...
  while (true) {
    ...
    if (millis() > (currentDelay + currentMillis)) {
      ...
      return 1; // CHANGE BACK TO return 0;
    }
...
int get_data_from_basys() {
  ...
  int verilog_state = 0;
  unsigned long currentMillis = millis();
  unsigned long currentDelay = 1000; // how long we will wait for a response from a beacon before moving on:
  ...
  while (true) {
    ...
    if (verilog_state != prev_state) {
      ...
      prev_state = verilog_state; // just invert the previous state
...
void reset_basys() {
  ...
  delay(2000);
  digitalWrite(reset_basys_pin, 1);
  delay(50);
  digitalWrite(reset_basys_pin, 0);
  delay(300);
...
void loop()
{
  ...
  //  AFTER EIGHT DATA SAMPLES ENTER THIS SECTION:
  //  6. FLIP ARRAY
